...
- 兼容 AMD 和 CommonJS，如果当前环境不支持这两种规范，则将模块导出到全局对象上。

## CommonJS

- Node.js 默认的模块化方案，无法在浏览器中直接使用。
- 通过 `require` 导入模块，通过 `module.exports` 导出模块。
- 模块解析发生在运行时（动态）。`require` 会在代码执行过程中动态加载、解析模块。因此 `require` 可以放在模块代码的任意位置，路径可以是变量。
...
- 模块解析发生在编译时（静态）。V8 引擎在编译阶段对 `import` 导入的模块建立静态的引用，在运行时不能更改。所有 `import` 语句都需要放在模块代码的开头，路径必须是静态字符串。基于此，ESM 可以更好地进行 Tree Shaking。
- 如果同一个模块被导入到多个其他位置，那么它的代码只会在第一次被导入时执行，生成导出，之后导入该模块都会共享相同的导出。
- `import.meta` 对象包含了模块的元数据信息，例如在 `type="module"` 的 script 标签中可以通过 `import.meta.url` 获取当前模块的 URL。
- `import()` 表达式可以动态导入模块，返回一个 Promise 对象。可以在模块代码的任意位置调用，路径可以是变量。

## CommonJS 和 ESM 区别

- CommonJS 是运行时层面的模块化方案；ESM 是语言层面的模块化方案。
- CommonJS 是同步加载；ESM 是异步加载，可以并行加载多个模块。
- CommonJS 是动态解析，导出拷贝；ESM 是静态解析，导出引用。
- Node.js 默认支持 CommonJS，在 Node.js V12+ 版本中支持 ESM；浏览器中支持 ESM，不支持 CommonJS。
- Node.js 中 `.js` 默认为 CommonJS 模块，CommonJS 模块使用 `.cjs` 后缀；ESM 模块使用 `.mjs` 后缀。
- Node.js 中 ESM 模块顶层作用域中不包含 `__filename`、`__dirname`、`NODE_PATH` 等变量以及一些特性。
- Node.js 中 ESM 模块不支持 native 模块导入，可以通过 `module.createRequire` 创建一个新的 `require` 函数进行导入。
- Node.js 中 CommonJS 默认是非严格模式；ESM 默认是严格模式。

## 模块入口

- 在 `package.json` 中可以通过 `main` 字段定义 CommonJS 模块的入口文件路径；通过 `module` 字段定义 ESM 模块的入口文件路径。
- `exports` 字段可以定义模块的导出方式，可以自定义多个导出规则，其优先级高于 `main` 和 `module` 字段。
...
- 以 ESM 规范编写代码。
- 使用 Babel 等编译器将 ESM 代码转换为 CommonJS 代码。
- Webpack 按照 CommonJS 规范实现了模块加载机制。

## Tree Shaking 原理

- 生成依赖关系图。打包工具在编译阶段解析模块之间的依赖关系，生成依赖关系图。
- 标记未使用的代码。遍历依赖关系图中的所有导出项，标记其中未被引用的导出项。
- 删除未使用的代码。删除被标记的导出项相关的代码。
- 生成最终的构建成果物。
